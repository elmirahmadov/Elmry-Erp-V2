...
export async function confirmPurchaseVoucher(id: number, companyId: number) {
  const response = await fetchNoCache(
    buildApiUrl(ENDPOINTS.PURCHASES.CONFIRM(id)),
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const data = await handleApiResponse<{ voucher: PurchaseVoucherRecord }>(
    response,
    "Sened tesdiqlenemedi",
  );

  return data.voucher;
}

export async function unconfirmPurchaseVoucher(id: number, companyId: number) {
  const response = await fetchNoCache(
    buildApiUrl(ENDPOINTS.PURCHASES.UNCONFIRM(id)),
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ companyId }),
