...
import PurchaseFilterDrawer, {
  countActivePurchaseFilters,
  emptyPurchaseFilters,
  type PurchaseDetailFilters,
} from "./components/PurchaseFilterDrawer";
...
export default function PurchasePage({ mode }: PurchasePageProps) {
  const fieldClass =
  ...
  const numberInputClass =
    "w-full border border-border rounded px-3 py-2 text-base bg-transparent text-foreground focus:outline-none focus:ring-2 focus:ring-ring [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none";
  ...
  const [filterDraft, setFilterDraft] = useState<PurchaseDetailFilters>(
    emptyPurchaseFilters(),
  );
  const [appliedFilters, setAppliedFilters] = useState<PurchaseDetailFilters>(
    emptyPurchaseFilters(),
  );
  ...
  const filteredVouchers = useMemo(() => {
    const serial = appliedFilters.serialNo.trim().toLowerCase();
    const supplier = appliedFilters.supplierName.trim().toLowerCase();
    const dateFrom = appliedFilters.dateFrom
      ? new Date(`${appliedFilters.dateFrom}T00:00:00`)
      : null;
    const dateTo = appliedFilters.dateTo
      ? new Date(`${appliedFilters.dateTo}T23:59:59`)
      : null;

    return vouchers.filter((voucher) => {
      if (voucher.type !== mode) {
        return false;
      }
      if (serial && !voucher.serialNo.toLowerCase().includes(serial)) {
        return false;
      }
      if (
        supplier &&
        !voucher.supplierName.toLowerCase().includes(supplier)
      ) {
        return false;
      }
      if (appliedFilters.branch && voucher.branch !== appliedFilters.branch) {
        return false;
      }
      if (
        appliedFilters.warehouse &&
        voucher.warehouse !== appliedFilters.warehouse
      ) {
        return false;
      }
      if (appliedFilters.status && voucher.status !== appliedFilters.status) {
        return false;
      }

      const createdAt = new Date(voucher.createdAt);
      if (dateFrom && !Number.isNaN(dateFrom.getTime()) && createdAt < dateFrom) {
        return false;
      }
      if (dateTo && !Number.isNaN(dateTo.getTime()) && createdAt > dateTo) {
        return false;
      }

      return true;
    });
  }, [vouchers, appliedFilters, mode]);

  const paginatedVouchers = filteredVouchers;
  const activeFilterCount = countActivePurchaseFilters(appliedFilters);

  const warehouseFilterOptions = useMemo(() => {
    const fromMap = Object.values(warehouseByBranch).flat();
    const fromVouchers = vouchers.map((voucher) => voucher.warehouse);
    return Array.from(
      new Set([...fromMap, ...fromVouchers].filter(Boolean)),
    ).sort((a, b) => a.localeCompare(b, "tr"));
  }, [warehouseByBranch, vouchers]);

  const statusFilterOptions = useMemo(() => {
    return Array.from(
      new Set(vouchers.map((voucher) => voucher.status).filter(Boolean)),
    ).sort((a, b) => a.localeCompare(b, "tr"));
  }, [vouchers]);
  ...
  useEffect(() => {
    ...
    loadData();
  }, [companyId, currentPage, searchTerm, mode]);
  ...
  const resetForm = (type: VoucherType = mode) => {
    ...
    >
      <div className="shrink-0">
        <PurchaseToolbar
          ...
          setSmartSearchTerm={setSearchTerm}
          onOpenFilters={() => {
            setFilterDraft({ ...appliedFilters });
            setIsFilterOpen(true);
          }}
          activeFilterCount={activeFilterCount}
          onCreate={openNewVoucher}
        />
        {pageError && (
          <p className="border-b border-border bg-card px-4 pb-3 text-sm text-destructive">
            {pageError}
          </p>
        )}
      </div>
      ...
      <div className="flex-1 min-h-0 overflow-auto">
        <table className="w-full min-w-[1180px] table-fixed border-collapse">
          <thead className="sticky top-0 z-10 bg-secondary">
            <tr>
              <th className="w-28 border-b border-border px-3 py-2 text-center text-xs font-semibold text-foreground">
                Sened No
              </th>
              <th className="w-40 border-b border-border px-3 py-2 text-center text-xs font-semibold text-foreground">
                Tedarikci
              </th>
              <th className="w-36 border-b border-border px-3 py-2 text-center text-xs font-semibold text-foreground">
                Filial
              </th>
              <th className="w-36 border-b border-border px-3 py-2 text-center text-xs font-semibold text-foreground">
                Anbar
              </th>
              <th className="w-48 border-b border-border px-3 py-2 text-center text-xs font-semibold text-foreground">
                Tarih Saat
              </th>
              <th className="w-20 border-b border-border px-3 py-2 text-center text-xs font-semibold text-foreground">
                Kalem
              </th>
              <th className="w-28 border-b border-border px-3 py-2 text-center text-xs font-semibold text-foreground">
                Toplam
              </th>
              <th className="w-24 border-b border-border px-3 py-2 text-center text-xs font-semibold text-foreground">
                Durum
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border bg-card">
            {paginatedVouchers.map((voucher) => {
              ...
              return (
                ...
                >
                  <td className="w-28 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-center text-sm font-medium text-card-foreground">
                    {voucher.serialNo}
                  </td>
                  <td className="w-40 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-center text-sm text-foreground">
                    {voucher.supplierName}
                  </td>
                  <td className="w-36 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-center text-sm text-foreground">
                    {voucher.branch}
                  </td>
                  <td className="w-36 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-center text-sm text-foreground">
                    {voucher.warehouse}
                  </td>
                  <td className="w-48 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-center text-sm text-foreground">
                    {formatDateTime(voucher.createdAt)}
                  </td>
                  <td className="w-20 px-3 py-2 text-center text-sm text-foreground">
                    {voucher.lines.length}
                  </td>
                  <td className="w-28 px-3 py-2 text-center text-sm font-semibold text-primary">
                    {formatCurrency(total)}
                  </td>
                  <td className="w-24 px-3 py-2 text-center text-sm text-foreground">
                    {voucher.status}
                  </td>
                </tr>
              );
      ...
      <div className="shrink-0 border-t bg-secondary px-4 py-3">
        <div className="flex items-center justify-between">
          <span className="text-sm text-muted-foreground">
            Toplam Satır:{" "}
            {activeFilterCount > 0 ? filteredVouchers.length : totalRows}
          </span>

          <div className="flex items-center gap-1">
            <button
              className="px-3 py-1 rounded border bg-card text-foreground hover:bg-accent disabled:opacity-50"
      ...
      {voucherMounted && (
        ...
        >
          ...
          <div
            className="relative flex h-full w-full max-w-5xl flex-col bg-card shadow-2xl transition-all duration-300 ease-in-out"
            style={{
          ...
          >
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="shrink-0 rounded bg-emerald-600 px-3 py-2 text-base font-medium text-primary-foreground hover:bg-emerald-700"
                    >
                      Sec
                    </button>
                  </div>
              ...
              <div className="mt-6">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="rounded bg-indigo-600 px-3 py-2 text-sm font-medium text-primary-foreground hover:bg-indigo-700"
                  >
                    + Urun Sec
                  </button>
                </div>

                <div className="overflow-x-hidden rounded border border-border">
                  <table className="w-full table-fixed border-collapse">
                    <thead className="bg-secondary">
                      <tr>
                        <th className="w-24 border-b px-3 py-2 text-left text-xs font-semibold text-foreground">
                          Kod
                        </th>
                        <th className="w-64 border-b px-3 py-2 text-left text-xs font-semibold text-foreground">
                          Urun
                        </th>
                        <th className="w-24 border-b px-3 py-2 text-left text-xs font-semibold text-foreground">
                          Birim
                        </th>
                        <th className="w-32 border-b px-3 py-2 text-left text-xs font-semibold text-foreground">
                          Miktar
                        </th>
                        <th className="w-32 border-b px-3 py-2 text-left text-xs font-semibold text-foreground">
                          Birim Fiyat
                    ...
                    <tbody className="divide-y divide-border bg-card">
                      {form.lines.map((line) => {
                        ...
                        return (
                          <tr key={line.id}>
                            <td className="w-24 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-sm text-foreground">
                              {product.code}
                            </td>
                            <td className="w-64 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-sm text-foreground">
                              {product.name}
                            </td>
                            <td className="w-24 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-sm text-foreground">
                              {product.unit}
                            ...
                            <td className="w-32 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-sm font-semibold text-primary">
                ...
                <div className="mt-3 flex justify-end">
                  <div className="rounded bg-primary/10 px-4 py-2 text-sm font-semibold text-primary">
            ...
            <div className="flex items-center justify-between border-t bg-secondary px-6 py-4">
              <div>
                {editingId &&
                  (deleteConfirm ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="rounded bg-destructive px-3 py-1.5 text-sm font-medium text-destructive-foreground hover:bg-destructive/90 disabled:opacity-60"
                      >
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="rounded bg-muted px-4 py-2 text-sm text-foreground hover:bg-muted"
                >
                  Iptal
                </button>
                <button
      ...
      {productMounted && (
        ...
        >
          ...
          <div
            className="relative z-10 rounded-xl bg-card shadow-2xl transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]"
            style={{
          ...
          >
            ...
            <div className="flex h-[calc(100%-81px)] min-h-0 flex-col px-5 py-4">
              <div className="grid min-h-0 flex-1 grid-cols-[220px_minmax(0,1fr)] gap-4">
                <aside className="flex min-h-0 flex-col rounded-xl border border-border bg-secondary/80 p-3">
                ...
                <div className="flex min-h-0 min-w-0 flex-col rounded-xl border border-border bg-card">
                  <div className="border-b border-border px-4 py-3">
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                      <input
                        ...
                        className="w-full rounded-lg border border-border px-3 py-2 text-sm outline-none focus:border-blue-500 lg:max-w-sm"
                      />
                    ...
                    <table className="w-full min-w-[1040px] table-fixed border-collapse">
                      <thead className="sticky top-0 bg-secondary">
                        <tr>
                          ...
                          <th className="w-72 border-b px-3 py-2 text-left text-xs font-semibold text-foreground">
                            Urun
                          </th>
                          <th className="w-48 border-b px-3 py-2 text-left text-xs font-semibold text-foreground">
                            Kategori
                          </th>
                          <th className="w-24 border-b px-3 py-2 text-left text-xs font-semibold text-foreground">
                            Birim
                      ...
                      <tbody className="divide-y divide-border bg-card">
                        ...
                        {paginatedProductResults.map((product) => (
                          ...
                          >
                            ...
                            <td className="w-72 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-sm text-foreground">
                              {product.name}
                            </td>
                            <td className="w-48 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-sm text-muted-foreground">
                              {product.category}
                            </td>
                            <td className="w-24 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-sm text-foreground">
                              {product.unit}
                            </td>
                            <td className="w-32 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-sm font-medium text-primary">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border bg-secondary px-4 py-3">
                    <span className="text-sm text-muted-foreground">
                    ...
                    <div className="flex flex-wrap items-center gap-1">
                      <button
                        type="button"
                        className="rounded border bg-card px-3 py-1 text-sm text-foreground disabled:opacity-50"
                        onClick={() =>
          ...
          >
            ...
            <div className="h-[calc(620px-58px)] px-5 py-4">
              <div className="grid h-full grid-cols-[240px_minmax(0,1fr)] gap-4">
                <aside className="h-full overflow-auto rounded-lg border border-border bg-secondary p-3">
                  <h4 className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                    Kategoriler
                  </h4>
                  <div className="space-y-1 overflow-auto pr-1">
                    {supplierCategories.map((category) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex w-full items-center justify-between rounded px-2 py-2 text-left text-sm transition ${
                            isActive
                              ? "bg-emerald-100 font-semibold text-success"
                              : "text-foreground hover:bg-accent"
                          }`}
                        >
                          <span>{category}</span>
                ...
                <div className="flex min-h-0 min-w-0 flex-col rounded-xl border border-border bg-card">
                  ...
                  <div className="min-h-0 flex-1 overflow-auto">
                    <table className="w-full min-w-[760px] table-fixed border-collapse">
                      <thead className="sticky top-0 bg-secondary">
                        <tr>
                          <th className="w-20 border-b px-3 py-2 text-left text-xs font-semibold text-foreground">
                            ID
                          </th>
                          <th className="w-96 border-b px-3 py-2 text-left text-xs font-semibold text-foreground">
                            Tedarikci Adi
                          </th>
                          <th className="w-64 border-b px-3 py-2 text-left text-xs font-semibold text-foreground">
                            Kategori
                      ...
                      <tbody className="divide-y divide-border bg-card">
                        ...
                        {paginatedSupplierResults.map((supplier) => (
                          <tr
                            key={supplier.id}
                            className="cursor-pointer transition-colors hover:bg-success/10"
                            onClick={() => selectSupplier(supplier.id)}
                          >
                            <td className="w-20 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-sm text-foreground">
                              {supplier.id}
                            </td>
                            <td className="w-96 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-sm font-medium text-foreground">
                              {supplier.name}
                            </td>
                            <td className="w-64 whitespace-nowrap overflow-hidden text-ellipsis px-3 py-2 text-sm text-muted-foreground">
                              {supplier.category}
                            </td>
                          </tr>
                        ))}
      ...
      <PurchaseFilterDrawer
        ...
        onClear={() => {
          const cleared = emptyPurchaseFilters();
