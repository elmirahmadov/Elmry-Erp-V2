...
interface CompanyTillsDrawerProps {
  isOpen: boolean;
...
export default function CompanyTillsDrawer({
  isOpen,
  companyId,
  companyName,
  onClose,
}: CompanyTillsDrawerProps) {
  const [items, setItems] = useState<Till[]>([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const list = await fetchTills(companyId);
  ...
  return (
    <SideDrawer
      isOpen={isOpen}
      onClose={onClose}
      title={companyName ? `Kasalar — ${companyName}` : "Kasalar"}
      widthClassName="max-w-xl"
    >
      <form onSubmit={onSubmit} className="mb-4 flex gap-2">
        <input
          className="wv-input flex-1"
          placeholder="Yeni kassa adi"
          value={name}
      ...
      {loading ? (
        ...
        <p className="text-sm text-muted-foreground">Henuz kassa yok.</p>
      ) : (
